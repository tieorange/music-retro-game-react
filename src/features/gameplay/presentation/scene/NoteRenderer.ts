...
class VisualNote extends Container {
    ...
    public setup(color: number, width: number) {
        this.drawGlow(color, width);
        this.drawBody(color, width);
    ...
    public drawGlow(color: number, width: number, alpha: number = 0.3) {
        this.glow.clear();
        this.glow.circle(0, 0, width / 2 + 1);
        this.glow.fill({ color, alpha });
        this.glow.circle(0, 0, width / 3);
        this.glow.fill({ color, alpha: alpha * 2 });
    }

    public drawBody(color: number, width: number) {
        this.body.clear();
        ...
        this.body.roundRect(-width / 2, -10, width, 20, 10);
        this.body.fill(grad);
        this.body.stroke({ color: 0xffffff, width: 2, alpha: 0.8 });
    }

    public updateTrail(color: number, length: number, width: number, isHold: boolean = false) {
        this.trail.clear();
        if (length <= 0) return;

        if (isHold) {
            this.trail.moveTo(-width / 4, 0);
            this.trail.lineTo(width / 4, 0);
            this.trail.lineTo(width / 4, -length);
            this.trail.lineTo(-width / 4, -length);
            this.trail.fill({ color, alpha: 0.6 });

            this.trail.rect(-width / 3, -length - 5, (width / 3) * 2, 10);
            this.trail.fill({ color: 0xffffff, alpha: 0.8 });
        } else {
            this.trail.moveTo(-width / 6, 0);
            this.trail.lineTo(width / 6, 0);
...
export class NoteRenderer extends Container {
    ...
    constructor(laneXPositions: number[], hitZoneY: number, spawnY: number, noteWidth: number = 48) {
        super();
    ...
    public update(currentTime: number, activeNotes: ActiveNote[]) {
        ...
        for (const note of activeNotes) {
            ...
            if (!vNote) {
                ...
                vNote.setup(color, this.noteWidth);
            ...
            if (timeUntilHit < 0.1 && timeUntilHit > -0.15) {
                ...
                if (flash) {
                    vNote.drawGlow(0xff0000, this.noteWidth, 0.8);
                } else {
                    vNote.drawGlow(color, this.noteWidth);
                }
            } else {
            ...
            vNote.updateTrail(color, trailLength, this.noteWidth, note.type === 'hold');
        }
