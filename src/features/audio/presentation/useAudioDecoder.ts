...
import { decodeAudioFile, decodeAudioUrl } from '@/features/audio/data/audioDecoder';

interface UseAudioDecoderResult {
    isDecoding: boolean;
    error: string | null;
    decode: (file: File) => Promise<AudioBuffer | null>;
    decodeUrl: (url: string, name: string) => Promise<AudioBuffer | null>;
}
...
export function useAudioDecoder(): UseAudioDecoderResult {
    ...
    const decodeUrl = async (url: string, name: string) => {
        setIsDecoding(true);
        setError(null);

        try {
            logInfo('audio.decode.started', { url: url.slice(-60), name });
            const t0 = performance.now();
            ...
            logError('audio.decode.failed', { name }, err);
            setError(err.message || 'Failed to decode audio');
            setIsDecoding(false);
            return null;
        }
    ...
    return { isDecoding, error, decode, decodeUrl };
}
